...
def update_status():
    global sdr_publisher, server_status, last_status, message_label
    sdr_publisher.setblocking(False)
    with app.app_context():
        while True:
            ...
            try:
                ...
                elif(last_status == server_status):
                    state = "ERROR!"
                    color = "red"
                else:
                    state = "OKAY!"
                ...
                print(server_status)
                turbo.push(turbo.replace(render_template('server_status.html', server_status_msg=state, message_log=message_label, state_color=color ), target="status"))
